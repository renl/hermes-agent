...
class HermesCLI:
    ...
    def _run_whatsapp_cli_chat_outreach(self, instruction_text: str) -> bool:
        from gateway.config import Platform, load_gateway_config
        from gateway.platforms.whatsapp import WhatsAppAdapter

        try:
            operator_instruction, run_request = (
                build_cli_chat_whatsapp_outreach_requests(
                    instruction_text,
                    session_id=self.session_id,
                )
            ...
            _cprint(
                f"  {_DIM}WhatsApp approved outreach rejected the request: {_escape(str(exc))}{_RST}"
            )
            return True
        ...
        try:
            ...
            _cprint(
                f"  {_DIM}Could not load WhatsApp gateway config: {_escape(str(exc))}{_RST}"
            )
            return True

        if not platform_config or not platform_config.enabled:
            _cprint(
                f"  {_DIM}WhatsApp is not configured or enabled in gateway config.{_RST}"
            )
            return True
        ...
        try:
            result = _run_async_in_thread(_run_outreach_flow())
            if result is None:
                _cprint(
                    f"  {_DIM}WhatsApp bridge is unavailable; cold-start send stayed blocked.{_RST}"
                )
                return True
        except Exception as exc:
            _cprint(
                f"  {_DIM}WhatsApp approved outreach failed: {_escape(str(exc))}{_RST}"
            )
            return True

        self._print_user_message_preview(instruction_text)
        self.conversation_history.append({
            "role": "user",
        ...
        self.conversation_history.append({"role": "assistant", "content": rendered})
        ChatConsole().print(
            Panel(
                _render_final_assistant_content(
                    rendered, mode=self.final_response_markdown
                ),
                title="[bold]⚕ Hermes[/]",
                title_align="left",
                border_style=_maybe_remap_for_light_mode("#CD7F32"),
                style=_maybe_remap_for_light_mode("#FFF8DC"),
                box=rich_box.HORIZONTALS,
                padding=(1, 4),
                width=self._scrollback_box_width(),
            )
            ...
            while not self._should_exit:
                try:
                    ...
                    if (
                        ...
                        and isinstance(user_input, str)
                        and not submit_images
                        and is_whatsapp_approved_outreach_instruction(user_input)
                    ):
                        self._run_whatsapp_cli_chat_outreach(user_input)
                        continue
