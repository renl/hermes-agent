...
def cmd_whatsapp(args):
    ...
    if getattr(args, "whatsapp_command", None) == "governance":
        parts = ["whatsapp", str(args.governance_action)]
        if getattr(args, "destination_key", None):
            parts.append(f"destination_key={args.destination_key}")
        if getattr(args, "conversation_key", None):
            parts.append(f"conversation_key={args.conversation_key}")
        if getattr(args, "group_chat_id", None):
            parts.append(f"group_chat_id={args.group_chat_id}")
        if getattr(args, "dm_counterparty_id", None):
            parts.append(f"dm_counterparty_id={args.dm_counterparty_id}")
        if getattr(args, "approved_destination_chat_id", None):
            parts.append(
                f"approved_destination_chat_id={args.approved_destination_chat_id}"
            )
        if getattr(args, "operator_objective", None):
            parts.append(
                f"operator_objective={json.dumps(args.operator_objective, ensure_ascii=False)}"
            )
        if getattr(args, "message_text", None):
            parts.append(
                f"message_text={json.dumps(args.message_text, ensure_ascii=False)}"
            )
        print(" ".join(parts))
        return
...
def main():
    ...
    whatsapp_governance = whatsapp_subparsers.add_parser(
        "governance",
        help="Print a Hermes-local WhatsApp governance instruction",
        description=(
            "Build a Hermes-local governance instruction for WhatsApp conversation "
            "start or revise flows. The emitted instruction is meant for Hermes-owned "
            "interactive chat surfaces, not as manual WhatsApp control traffic."
        ),
    )
    whatsapp_governance.set_defaults(func=cmd_whatsapp)
    whatsapp_governance.add_argument(
        "governance_action",
        choices=("start", "revise"),
        help="Governance action to encode for the local interactive surface",
    )
    whatsapp_governance.add_argument("--destination-key")
    whatsapp_governance.add_argument("--conversation-key")
    whatsapp_governance.add_argument("--group-chat-id")
    whatsapp_governance.add_argument("--dm-counterparty-id")
    whatsapp_governance.add_argument("--approved-destination-chat-id")
    whatsapp_governance.add_argument("--operator-objective", required=True)
    whatsapp_governance.add_argument("--message-text")
