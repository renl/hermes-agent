...
def test_prompt_submit_routes_tui_local_governance_into_outreach_workflow(monkeypatch):
    class _Agent:
        def run_conversation(self, *args, **kwargs):
            raise AssertionError("agent.run_conversation should not be called")
    ...
    monkeypatch.setattr(
        ...
        lambda: types.SimpleNamespace(
            platforms={Platform.WHATSAPP: types.SimpleNamespace(enabled=True)}
        ),
    )
    monkeypatch.setattr(
        "gateway.platforms.whatsapp.WhatsAppAdapter", lambda _cfg: _Adapter()
    )
    ...
    async def _fake_execute(run_request, *, authorized, adapter):
        ...
        return {
            ...
            "run": {
                "operator_ingress_surface": run_request["operator_ingress_surface"],
            ...
            "plan": {
                ...
                "conversation_channel_mode": "conversational_primary",
                "operator_governance_policy": "owner_approved_conversation",
                "operator_objective": "Request the first quote",
            },
            "execution": {
                "interaction_lane": "external_conversation",
                "resolved_target": {
                    "destination_chat_id": "15551230000@s.whatsapp.net"
                },
                "message_generation_mode": "operator_text_override",
            ...
            "founder_summary": "Founder-visible ingress ran through Hermes-local governance.",
        }

    monkeypatch.setattr(server, "execute_whatsapp_approved_outreach", _fake_execute)
    ...
    assert "operator_ingress_surface: tui_chat" in payload["text"]
    assert captured["run_request"]["operator_ingress_surface"] == "tui_chat"
    assert captured["run_request"]["interaction_lane"] == "operator_governance"
    assert server._sessions["sid"]["history"][0]["role"] == "user"
    assert server._sessions["sid"]["history"][1]["role"] == "assistant"


def test_prompt_submit_routes_dashboard_local_governance_into_outreach_workflow(
    monkeypatch,
):
    class _Agent:
        def run_conversation(self, *args, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        lambda result: (
            f"operator_ingress_surface: {result['run']['operator_ingress_surface']}"
        ),
    )
    ...
    assert captured["run_request"]["operator_ingress_surface"] == "dashboard_chat"
    assert server._sessions["sid"]["history"][0]["role"] == "user"
    assert server._sessions["sid"]["history"][1]["role"] == "assistant"
